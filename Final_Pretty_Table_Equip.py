...
headers = {
        ...
        'authorization': "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9..2LgRnRzNIIxKVXWRKwrBn4RK450r0TrkdyX2JfRJ1qDOGYJkIsXlI6rJHDjc6JpE7c0oPrxbX901DwXaL62INA"
}
        ...
        for values in clan_list1.values(): ############### Gets the keys from the dict in clash_list_dict.py
            ...
            if user_json['name']: ######################## Pulls the name key out and provides the name and prints it to the file.
                with open('player_equipment.txt', 'a', encoding='utf-8') as f:
                    ...
                    print("'Player', 'Equip', 'Current', 'Max'", file=f)
            for items in equip: ################## Pulls the equip value out for the name and prints it to the file. 
                with open('player_equipment.txt', 'a', encoding='utf-8') as f:
                     print(f"'{user_json['name']}','{items['name']}', '{items['level']}','{items['maxLevel']}'", file=f)
        table = PrettyTable()
        from prettytable import from_csv
        fp = open("player_equipment.txt", "r")
        table = from_csv(fp)
        table.align = "l"
        with open('Player_Table.txt', 'a', encoding='utf-8') as f:
            print(table, file=f)
            fp.close()
all_user()
